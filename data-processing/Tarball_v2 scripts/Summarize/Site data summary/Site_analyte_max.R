...
var_max <- function(site, tar) {
  ...
  var.max <- as.data.frame(df %>% summarise_if(is.numeric, max)) %>% mutate_if(is.numeric, round, 1)
  
  #Set "Site" as column name
  row.names(var.max) <- site
...
site.data_max <- do.call(rbind, lapply(data.sets, var_max, tarball))

#Export summary table as csv
date <- format(Sys.Date(), "%m-%d-%y")
write.csv(site.data_max, paste0("SOM_Site_data_max_",date,".csv"), row.names=TRUE)
