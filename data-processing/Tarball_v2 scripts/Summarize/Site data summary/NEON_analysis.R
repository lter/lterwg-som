...
# Will Wieder
# Nov, 2019
# quick look at NEON data products
#########################
rm(list=ls())
library(ggplot2)
library(tidyverse)
...
tarball$observation_date <- str_replace(tarball$observation_date, 
                                        "Aug-Sep 2011", "2011")
tarball$lyr_soc_stock
unique(tarball$google_dir)
print(unique(tarball$soc_stock))
tarball$observation_date['Aug-Sep 2011'] = tarball$observation_date['2011']  
NEONds    <- tarball %>% filter(network == 'NEON')
print(unique(NEONds$data_file))
NEONlit <- NEONds %>%
  filter(data_file%in%c("litterfall_annual_with_chem"))
...
print(NEONMegaRoots$bgb_c_stock)
# Putting roots and soil together into one column
NEONMega <- bind_rows(NEONMegaRoots, NEONMegaSoil) %>%
