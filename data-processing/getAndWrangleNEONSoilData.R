...
if (file.exists('/Users/sweintraub/')){
  ...
  dir2 <- ("/Users/sweintraub/Documents/Conferences_Meetings/2018_LTER-SOM")
  setwd(dir)
}
...
soilProds <- subset(products, grepl("soil", products$productName, ignore.case = TRUE), 
                    select=c("productCode", "productName")) 
soilProds # Pick out the DPs you want, then down below


### Download and 'stack' site-month files using the neonUtilities package
# devtools::install_github("NEONScience/NEON-utilities/neonUtilities", dependencies=TRUE) # install package if needed 
...
# Soil physical properties (Megapit), DP1.00096.001
{
  ...
  stackByTable(paste0(getwd(), "/filesToStack00096"), folder=T, saveUnzippedFiles = F)
}
# Soil chemical properties (Megapit), DP1.00097.001
{
  zipsByProduct(dpID="DP1.00097.001", site="all", package="basic", check.size=F)
  stackByTable(paste0(getwd(), "/filesToStack00097"), folder=T, saveUnzippedFiles = F)
}
# Soil chemical properties (Distributed periodic), DP1.10078.001
{
  ...
  stackByTable(paste0(getwd(), "/filesToStack10078"), folder=T, saveUnzippedFiles = F)
}  
# Soil physical properties (Distributed periodic), DP1.10086.001 - takes a while
{
  zipsByProduct(dpID="DP1.10086.001", site="all", package="basic", check.size=F)
  stackByTable(paste0(getwd(), "/filesToStack10086"), folder=T, saveUnzippedFiles = F)
}  
# Soil chemical properties (Distributed initial characterization), DP1.10008.001
{
  ...
  stackByTable(paste0(getwd(), "/filesToStack10008"), folder=T, saveUnzippedFiles = F)
}  
# Soil physical properties (Distributed initial characterization), DP1.10047.001
{
  ...
  stackByTable(paste0(getwd(), "/filesToStack10047"), folder=T, saveUnzippedFiles = F)
}  
# Soil stable isotopes (Distributed periodic), DP1.10100.001
{
  ...
  stackByTable(paste0(getwd(), "/filesToStack10100"), folder=T, saveUnzippedFiles = F)
}  
# Soil inorganic nitrogen pools and transformations, DP1.10080.001
{
  ...
  stackByTable(paste0(getwd(), "/filesToStack10080"), folder=T, saveUnzippedFiles = F)
}  
...
sls <- def.extr.geo.os(slsLocations, 'namedLocation') # takes a while
# export csv files
...
# Order by pit, then by horizon top depth
megpit_biogeo_3 <- arrange(megpit_biogeo_3, pitID, biogeoTopDepth)
# Write the file, customize path
write.csv(megpit_biogeo_3, paste(dir2, "megapit_all.csv", sep = "/"), row.names = F)
...
# Order by site, then by horizon top depth
spc_biogeo_3 <- arrange(spc_biogeo_3, pitID, biogeoTopDepth)
# Write the file
write.csv(spc_biogeo_3, paste(dir2, "intitalChar_all.csv", sep = "/"), row.names = F)
...
primaryKey))
# Write the file, customize path
write.csv(sls_all_clean, paste(dir2, 'periodic_all.csv', sep = "/"), row.names = F)


### Optional - Download other input data for Soil C modeling - litterfall and roots
# Litterfall and fine woody debris sampling, DP1.10033
{
  zipsByProduct(dpID="DP1.10033.001", site="all", package="basic", check.size=F)
  stackByTable(paste0(getwd(), "/filesToStack10033"), folder=T, saveUnzippedFiles = F)
}
...
###########################################################
### NEON-API direct interaction wihtout helper functions, just to demo
